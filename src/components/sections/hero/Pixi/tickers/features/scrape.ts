...
export default async function scrape(props: Props) {
    ...
    await Promise.all(
        [
            [0, 12, 13, 14].map((index) => dots[index].animate({ x: 30, y: 30 }, { delay: 0.2 })),
            ...
            [3, 21, 22, 23].map((index) =>
                dots[index].animate({ x: CELL_SIZE - 30, y: CELL_SIZE - 30 }, { delay: 0.2 })
            ),
    ...
    await Promise.all(
        [
            ...
            [1, 15, 16, 17].map((index) => dots[index].animate({ x: CELL_SIZE - 22, y: 22 })),
            [2, 18, 19, 20].map((index) => dots[index].animate({ x: 22, y: CELL_SIZE - 22 })),
            [3, 21, 22, 23].map((index) =>
            ...
            rects.slice(4, 8).map((rect) => rect.animate({ x: CELL_SIZE - 31, y: 31 })),
            rects.slice(8, 12).map((rect) => rect.animate({ x: 31, y: CELL_SIZE - 31 })),
            rects
                .slice(12, 16)
    ...
    Promise.all(dots.map((dot) => dot.animate({ alpha: 0 }, { delay: Math.random() * 0.3 })));
    ...
    await Promise.all([
        rects.map((rect, index) => {
            ...
            return rect.animate(
                {
                    y,
                    x: (index % 2 === 0 ? 8 : newWidths[index - 1] + 10) + newWidths[index] / 2,
                    height: 4,
