...
export default async function search(props: Props) {
    ...
    for await (const rect of rects) {
        ...
        const surroundingDots = dots.filter((dot) => {
            ...
            return (dx === 0 && dy === 9) || (dx === 9 && dy === 0) || (dx === 9 && dy === 9);
        });

        await Promise.all(
            [
                surroundingDots.map((dot) => dot.animate({ alpha: 1 }, { duration: 0.75 })),
                rect.animate({ alpha: 1, width: 14, height: 14 }, { duration: 0.75 }),
            ].flat()
        );

        rect.animate({ alpha: 0.68, width: 10, height: 10 }, { duration: 0.75 });
        Promise.all(surroundingDots.map((dot) => dot.animate({ alpha: 0.68 }, { duration: 0.75 })));
    }
